...
class GenerateSitemap(http.Controller):
    ...
    def index(self, **kw):
        ...
        #blogs = checkModule('website_blog', 'blog.blog', website_id, 'blogs')
        ...
        for page in web_pages:
            p = {'name': page['name'].title().replace('-', ' ').rstrip(), 'url': page['url']}
...
def getWebsitePages(clean_pages, enum_urls):
    ...
    for url in enum_urls:
        ...
        if len(split) == 2 and split[1] != "":
            temp_dict = {'name' : split[1].title().replace('-', ' ').rstrip(), 'url' : url}
